...
launcher="${app_dir}/scripts/start-red-scheduled-x11.sh"
...
sudo systemctl disable --now gk-red-kiosk.service 2>/dev/null || true
sudo systemctl disable --now gk-taplister-red.service 2>/dev/null || true
sudo systemctl daemon-reload
sudo systemctl restart getty@tty1.service

echo "Configured tty1 autologin + red X11 scheduled kiosk."
