...
# Usage:
#   ./scripts/install-blue-service.sh            # defaults to kmsdrm
#   ./scripts/install-blue-service.sh offscreen  # fallback path
driver="${1:-kmsdrm}"
...
user_name="$(id -un)"
service_name="gk-taplister-blue.service"
service_path="/etc/systemd/system/${service_name}"

sudo tee "$service_path" >/dev/null <<EOF
[Unit]
Description=GK Taplister Blue
...
ExecStart=/usr/bin/python3 ${app_dir}/blue-side.py
Restart=always
RestartSec=2
